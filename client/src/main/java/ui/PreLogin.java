...
public class PreLogin {
        ...
        while (true) {
            ...
            try {
                ...
                System.out.println(EscapeSequences.SET_TEXT_COLOR_RED +
                        "Error: " + e.getMessage() + EscapeSequences.RESET_TEXT_COLOR);
            }
        }
    }
    ...
    private void handleRegister(String[] tokens) throws Exception {
        ...
        RegisterRequest req = new RegisterRequest(tokens[1], tokens[2], tokens[3]);
    ...
    private void handleLogin(String[] tokens) throws Exception {
        ...
        LoginRequest req = new LoginRequest(tokens[1], tokens[2]);
        LoginResult result = facade.login(req);

        if (result.getMessage() == null) {
            System.out.println(EscapeSequences.SET_TEXT_COLOR_GREEN +
                    "Login successful." + EscapeSequences.RESET_TEXT_COLOR);
            new PostLogin(tokens[1], result.getAuthToken()).run();
        } else {
            System.out.println(EscapeSequences.SET_TEXT_COLOR_RED +
